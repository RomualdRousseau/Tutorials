...
from tutorial1.math.linalg import almost, lst_2_np
...
def test_point_nearest_with_segment():
    ...
    assert x.almost(Point(lst_2_np([1.5, 1.5])))
...
def test_point_nearest_out_of_segment_but_non_strict():
    ...
    assert x is not None
    assert x.almost(Point(lst_2_np([1, 1])))
    x = nearest_point_segment(d, e, True)
    assert x is not None
    assert x.almost(Point(lst_2_np([2, 2])))
